#ifndef MBO_FILTER_MBO_FILTER_H_
#define MBO_FILTER_MBO_FILTER_H_
...
namespace MBOfilter {
// status returned by a MBO filter operation
enum Status {
  Ok = 0,
  NotFound = 1,
  NotEnoughSpace = 2,
...
// maximum number of MBO kicks before claiming failure
const size_t kMaxMBOCount = 500;

// A MBO filter class exposes a Bloomier filter interface,
...
class MBOFilter {
 ...
 public:
  explicit MBOFilter(const size_t max_num_keys) : num_items_(0), victim_() {
    size_t assoc = 4;
...
Status MBOFilter<ItemType, bits_per_item, TableType>::Add(
    const ItemType &item) {
  size_t i;
  uint32_t tag;
...
template <typename ItemType, size_t bits_per_item,
          template <size_t> class TableType>
Status MBOFilter<ItemType, bits_per_item, TableType>::AddImpl(
    const size_t i, const uint32_t tag) {
  size_t curindex = i;
  uint32_t curtag = tag;
  uint32_t oldtag;

  for (uint32_t count = 0; count < kMaxMBOCount; count++) {
...
template <typename ItemType, size_t bits_per_item,
          template <size_t> class TableType>
Status MBOFilter<ItemType, bits_per_item, TableType>::Contain(
...
template <typename ItemType, size_t bits_per_item,
          template <size_t> class TableType>
Status MBOFilter<ItemType, bits_per_item, TableType>::Delete(
    const ItemType &key) {
  size_t i1, i2;
  uint32_t tag;
...
template <typename ItemType, size_t bits_per_item,
          template <size_t> class TableType>
std::string MBOFilter<ItemType, bits_per_item, TableType>::Info() const {
  std::stringstream ss;
  ss << "MBOFilter Status:\n"
...
#endif  // MBO_FILTER_MBO_FILTER_H_
